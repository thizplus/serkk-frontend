...
function ChatInputComponent({
  ...
  const handleFileSelect = useCallback((e: ChangeEvent<HTMLInputElement>, type: "image" | "file") => {
    ...
    if (selectedFiles.length + files.length > maxFiles) {
      toast.error(TOAST_MESSAGES.MEDIA.TOO_MANY_FILES);
      return;
    }
    ...
    Array.from(files).forEach((file) => {
      ...
      if (type === "image") {
        ...
        if ((hasVideo && !isVideo) || (!hasVideo && isVideo && selectedFiles.length > 0)) {
          toast.error(TOAST_MESSAGES.MEDIA.MIXED_MEDIA_NOT_ALLOWED);
          return;
        }
        // Video limit: 1 only
        if (isVideo && (selectedFiles.length > 0 || files.length > 1)) {
          toast.error(TOAST_MESSAGES.MEDIA.ONE_VIDEO_ONLY);
          return;
        }
        // Max size
        const maxSize = isVideo ? 500 * 1024 * 1024 : 20 * 1024 * 1024; // 500MB for video, 20MB for image
        if (file.size > maxSize) {
          toast.error(TOAST_MESSAGES.MEDIA.FILE_TOO_LARGE);
          return;
        }
      } else {
        // File type

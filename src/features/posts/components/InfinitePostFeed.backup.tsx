...
import { OptimisticPostCard } from "./OptimisticPostCard"; // ✅ Phase 1: ใช้ full post view แทน compact
...
export function InfinitePostFeed({
  ...
  if (!posts || posts.length === 0) {
    // ถ้ามี optimistic posts อยู่ → แสดง optimistic posts + empty state
    if (enableOptimisticUI && optimisticPosts.length > 0) {
      return (
        <div className="space-y-4">
          {/* ✅ Phase 1: แสดง optimistic posts แบบ full view (Facebook-style) */}
  ...
  return (
    <div className="space-y-4">
      {/* ✅ Phase 1: Optimistic Posts (แสดงแบบ full view เหมือน real posts) */}
